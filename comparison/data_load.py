...
def load_wishbone():
    meta, data = fcsparser.parse(
        ...
        os.path.join(os.environ['HOME'], "data", "datasets",
...
def load_frey():
    ...
    ff = loadmat(os.path.join(os.environ['HOME'], "data", "datasets",
                              "frey_rawface.mat"),
                 squeeze_me=True,
...
def load_half_circles():
    data = loadmat(os.path.join(
        os.environ['HOME'], "data", "datasets", "halfCircles.mat"))
    colors, labels = pd.factorize(data["C"].reshape(-1))
...
def load_gmm_connected():
    data = loadmat(os.path.join(
        os.environ['HOME'], "data", "datasets", "gmmPHATE_connected.mat"))
    # data = loadmat(os.path.join(os.environ['HOME'], "data",
...
def load_gmm():
    data = loadmat(os.path.join(
        os.environ['HOME'], "data", "datasets", "gmmPhate.mat"))
    colors, labels = pd.factorize(data["labels"].reshape(-1))
...
def load_bonemarrow():
    clusters = pd.read_csv(os.path.join(
        os.environ['HOME'], "data", "datasets", "bmmc", "MAP.csv"), header=None)
...
def load_EB():
    EB_data = loadmat(os.path.join(
        os.environ['HOME'], "data", "datasets", "EB", "EBdata.mat"))
...
def load_ziesel():
    data = loadmat(os.path.join(
        os.environ['HOME'], "data", "datasets", "StenData.mat"))["data"]
    # C = loadmat(
    #     os.path.join(os.environ['HOME'], "data", "datasets",
...
def load_ipsc():
    data = loadmat(os.path.join(
        os.environ['HOME'], "data", "datasets", "ipscData.mat"))
...
def load_retinal_bipolar():
    ...
    with h5py.File(os.path.join(os.environ['HOME'], "data", "datasets",
                                "shekhar_retinal_bipolar",
...
def load_haber():
    data = loadmat(os.path.join(
        os.environ['HOME'], "data", "datasets", "intenstine_data.mat"))
